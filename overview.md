...
│   ├── agent/
│   │   ├── __init__.py
│   │   ├── orchestrator.py  # Manages agentic workflow (LangChain etc.)
│   │   ├── reasoning.py     # Handles chain-of-thought planning & math
│   │   └── approval.py      # User review/edit/approval logic
│   ├── llm/
│   │   ├── __init__.py
│   │   ├── llm_client.py    # API/local LLM interface (OpenAI, DeepSeek, etc.)
│   │   └── rag.py           # Retrieval-Augmented Generation: SKiDL docs/examples, part docs
│   ├── kicad/
│   │   ├── __init__.py
│   │   ├── skidl_interface.py  # SKiDL code generation & schematic/PCB file output
│   │   ├── part_search.py      # Wraps SKiDL's Part.search(), user part selection logic
│   │   └── pcb_tools.py        # Handles .kicad_pcb file ops, calls DeepPCB API for autorouting
│   ├── utils/
│   │   ├── __init__.py
│   │   ├── config.py        # Settings management, environment config
│   │   ├── file_ops.py      # File storage, download/export helpers
│   │   └── logging.py       # Unified logging/trace for transparency
│   └── data/
│       ├── skidl_docs/      # Indexed SKiDL docs and code snippets for RAG
│       └── example_designs/ # Reference SKiDL designs for retrieval
│
├── frontend/
│   ├── streamlit_app.py     # MVP/Prototype UI
│   └── react_app/           # Placeholder for future React/Next.js frontend
│
├── tests/
│   ├── test_agent.py        # Unit/integration tests for agentic workflow
│   ├── test_llm.py
│   ├── test_skidl.py
│   ├── test_part_search.py
│   └── ...                  # etc...
...
├── Dockerfile               # Containerize full stack (KiCad, SKiDL, FastAPI, etc.)
├── README.md
```
...
- **LLM:** DeepSeek-R1, OpenRouter, or local (API/CLI switchable)
